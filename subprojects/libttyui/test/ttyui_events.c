...
capture_handler(struct TtyUi *ui, struct TtyUiEvent *event, void *user_data) {
        ...
        ASSERT_TRUE(capture->count < (sizeof(capture->events) / sizeof(capture->events[0])));
...
static void
init_ttyui(struct TtyUi *ui, int *master_fd, int *slave_fd,
...
test_ttyui_utf8_key_sequence() {
        ...
        write_and_process(master_fd, &ui, &capture, euro_symbol,
                        strlen(euro_symbol));
