...
static bool
node_merge_while_left_cb(const struct RopeNode *node, void *userdata) {
	(void)userdata;
	size_t size;
	const uint8_t *data = rope_node_value(node, &size);
	return isupper(data[0]);
...
static void
test_node_merge_while_left(void) {
	int rv = 0;
	struct RopePool pool = {0};
	...
	struct RopeNode *h_node = rope_node_first(root);

	rope_node_merge_while(h_node, &pool, node_merge_while_left_cb, NULL);

	ASSERT_JSONEQ("['HELL','o']", root);
...
TEST(test_node_merge_while_left)
END_TESTS
