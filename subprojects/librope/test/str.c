...
test_str_init(void) {
	...
	rv = rope_str_init(&str, (const uint8_t *)"HelloWorld", 10, 0);
	ASSERT_EQ(rv, 0);
...
test_str_heap(void) {
	...
	rv = rope_str_init(&str, buffer, sizeof(buffer), 0);
	ASSERT_EQ(rv, 0);
...
test_str_heap_split_to_inline_both(void) {
	...
	struct RopeStr new_str = {0};
	rope_str_split(&str, &new_str, ROPE_STR_INLINE_SIZE);
...
test_str_heap_split_to_inline_right(void) {
	...
	struct RopeStr new_str = {0};
	rope_str_split(&str, &new_str, ROPE_STR_INLINE_SIZE + 1);
...
test_str_should_merge_utf8_sequence(void) {
	...
	rv = rope_str_init(&str1, buffer, 2, 0);
	ASSERT_EQ(rv, 0);

	size_t byte_size = rope_str_byte_size(&str1);
	ASSERT_EQ(2, byte_size);
	ASSERT_EQ(SIZE_MAX, rope_str_char_size(&str1));

	rv = rope_str_init(&str2, &buffer[2], 2, 0);
	ASSERT_EQ(rv, 0);

	byte_size = rope_str_byte_size(&str2);
	ASSERT_EQ(2, byte_size);
	ASSERT_EQ(SIZE_MAX, rope_str_char_size(&str2));
...
test_str_should_merge_grapheme(void) {
	...
	rv = rope_str_init(&str1, buffer, split, 0);
	ASSERT_EQ(rv, 0);

	rv = rope_str_init(&str2, &buffer[split], sizeof(buffer) - 1 - split, 0);
	ASSERT_EQ(rv, 0);
